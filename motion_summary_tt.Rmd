...
for(x in 1:length(percents)){
  ...
  estimates2 %>%
    ...
    geom_point(shape=19, size=0.0001, alpha = 0.2) + 
...
for(x in 1:length(percents)){
  ...
  estimates2 %>%
    ...
    geom_point(shape=19, size=0.0001, alpha = 0.4) + 
    geom_abline(intercept=0, slope=1) + 
    facet_grid(~motion_type, scales = "free") +
    theme_minimal() +
    ggtitle(#"Estimated Motion vs. Ground Truth Motion",
    ...
    ylim(-10, 10) +
    guides(colour = guide_legend(override.aes = list(alpha = 1, size=1))) +
    theme_minimal() -> scatterplots_abcd[[x]]
...
for(x in 1:length(percents)){
  ...
  estimates2 %>%
    ...
    ylim(-10, 10) +
    guides(colour = guide_legend(override.aes = list(alpha = 1, size=1))) +
    theme_minimal() -> scatterplots_hcp[[x]]
...
wrap_plots(p_abcd, p_hcp, ncol = 2) +
  plot_layout(guides = "collect") +
  plot_annotation(
