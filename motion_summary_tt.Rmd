...
estimates %>%
  ...
  filter(was.moved) %>%
  extract(estimated, c("motion.type", "axis"), "([[:alnum:]]+)_([[:alnum:]]+)") %>%
  ggplot(., aes(x=true_values, y=estimated_values, color=method, shape=as.factor(percent))) + 
