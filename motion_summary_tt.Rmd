...
estimates %>%
  ...
  ggplot(
    aes(x=true_values, 
        y=estimated_values, 
        color=method,
        # we may not need percent motion yet
        shape=as.factor(percent_motion)
        )
    ) +
  geom_point(alpha=0.1) + 
  geom_abline(intercept=0, slope=1) + 
  facet_grid(motion_type~ axis) +
...
absolute_error_hists <- vector("list", length = length(percents))
...
for(x in 1:length(percents)){
  ...
  absolute_error_hists[[x]] <- pl
}

### NOT NEEDED
# comparable %>%
#   # do the percent motion filter here
#   filter(percent_motion==15) %>%
#   
#   # start the ggplot
#   ggplot(aes(x=error, fill=method)) + 
#   geom_histogram(alpha=0.6, position="identity") +
#   facet_grid(scheme~motion.type, scales = "free_y") +
#   theme_cowplot(12) -> ss15
# 
...
plot_grid(plotlist = absolute_error_hists, labels = LETTERS[1:length(absolute_error_hists)], label_size = 12)
